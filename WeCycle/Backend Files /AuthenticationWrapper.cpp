...
const char* createAndRegisterAccount_account(const void *auth, const void *account) {
	Authentication *authObject = (Authentication *)auth;
    const char *result = authObject->createAndRegisterAccount((Account *)account).c_str();
	return result;
}
const char* createAndRegisterAccount_string(const void *auth, const char* email, const char* password) {
	Authentication *authObject = (Authentication *)auth;
    std::cout<< "EEEE" << std::endl;
	return authObject->createAndRegisterAccount(email, password).c_str();
}

const char* signInUser_account(const void *auth, const void  *account) {
	Authentication *authObject = (Authentication *)auth;
    return authObject->signInUser((Account *)account).c_str();
}

const char* signInUser_string(const void *auth, const char* email, const char* password) {
	Authentication *authObject = (Authentication *)auth;
